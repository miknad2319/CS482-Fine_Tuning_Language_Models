...
import re
#### we are not using nltk here as streamlit is having trouble installing it
...
##########################################################################################
#### // In order to load a model's state dict, we need to have the model class defined locally. 
#### // Due to the nature of torch, our model requires multiple stacked classes. This marks the
#### // beginning of the preprocessing code / modules required to instantiate our pretrained model
##########################################################################################
...
device = "cuda:0" if torch.cuda.is_available() else "cpu"# cuda because we are going to use a linux environment when we deploy our app on streamlit
...
model.load_state_dict(torch.load("model_state_dict.pt")) ## loading in the model state dictionary
model.eval()

##########################################################################################
#### // In order to load a model's state dict, we need to have the model class defined locally. 
#### // Due to the nature of torch, our model requires multiple stacked classes. This marks the
#### // end of the preprocessing code / modules required to instantiate our pretrained model
##########################################################################################
...
#### run an instance of comment text through the specified tokenizer and return its encoding
...
# defining what happens at inference time
def run_inference(encoding):
  with torch.no_grad():#disable the gradient process
    ...
    final_output = torch.softmax(output[1][0],dim=0).cpu()#softmax, find the top ine
...
#pre initialize a tokenized tweet
...
# The index of choice in model_pointers will access the models list
    ...
    # once the user picks a model and hits "analyze" we run
    # the encoding through the model and display the sentiment
    # and the score
